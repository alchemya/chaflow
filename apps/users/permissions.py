__author__ = 'yuchen'
__date__ = '2018/12/4 00:57'

from django.core.urlresolvers import resolve
from django.shortcuts import render,redirect,HttpResponse
from .permission_list import perm_dic
...
#     5.拿匹配到可权限key, 调用user.has_perm(key)
            ...
            if per_method == request.method: #matches request method
                ...
                for k,v in perm_kwargs.items():
                    ...
                    if arg_val == str(v): #匹配上了特定的参数 及对应的 参数值， 比如，需要request 对象里必须有一个叫 user_id=3的参数
                        ...
                        kwargs_matched = False
                        break # 有一个参数不能匹配成功，则判定为假，退出该循环。
                else:# 当列表为空的时候默认会走到这里
                    kwargs_matched = True

                #自定义权限钩子,很多时候,权限是和数据库查询相伴相生的，对此，我们需要相关的钩子函数，执行特殊操作， 通过返回true或者false来得到是否具备权限的结果
...
def check_permission(func):
    def inner(*args,**kwargs):
        if not perm_check(*args,**kwargs):
            request = args[0]
            return render(request,'xadmin/page_403.html')
